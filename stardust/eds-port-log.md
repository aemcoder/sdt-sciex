...
## 8. Phase 2 — delivery + published-origin gate (2026-09-18, agent g5)

Delivered via DA Source API (`deploy-batch.mjs`, ledger `content/.deploy-ledger.json`): `/events`, `/resource-hub`, `/resource-hub/regulatory-documents`, `/about-us/contact-us` — PUT 201 / preview 200 / live 200 each; 18 media PUT 201 to `media/sx/`. `localize-links` (CHECK PASS) also rewrote links in nav/footer/footer-legacy/index/pharma → redelivered (`--force`). Guard on the published URLs: all blocks loaded and grid/flex computed, `body.sx-port`, 0 pageerrors, 0 broken / zero-width images, listing rows from the snapshots (data 200), drives pass. `.plain.html`: 200 ×4; events and contact-us have no server-side `<h1>` (their blocks build it client-side — same as the source). AI-readability code score: events 6 (client-rendered listing), resource-hub 100, regulatory 98, contact-us 81 (country finder) — logged, not fixed.

Published gate (live sciex.com — THEIR EDS header/footer — vs ours): header +41 px / footer +120 px are chrome deltas by design (our v3 chrome is canonical); the gate is the body band anchored on the first content section. 1440 body: 0.94 % / 2.15 % / 1.56 % / 1.92 %; 360: 6.62 % / 3.90 % / 2.64 % / 5.22 %. Body height deltas found and fixed in ONE reconcile round (verified on the harness, pending commit): our base `img/svg { display:block }` leaking into the ported blocks (+31 px workflow icons, +7 px per event card), the empty metadata section eating the `:first-of-type` margin reset (+40 px), my `letter-spacing: normal` on nested paragraphs (+1 line), the static "Geogrotesque MD" being narrower than the VF alias (`size-adjust: 94.5 %`), the source's global `.search-wrapper { flex-wrap }` the contact block relies on, and the shim's facet cap (5 → 20, live lists every value). Numbers, evidence paths and residuals: `stardust/replica/progress-g5.json`.
